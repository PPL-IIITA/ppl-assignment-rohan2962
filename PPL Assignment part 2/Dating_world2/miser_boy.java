...
public class miser_boy extends boy {
     ...
     */
    public miser_boy(String nam, int a, int m, int b, int i) {
        super(nam, a, m, b, i);
    }
    ...
    public void calculate_happiness() {
        happiness_level = Math.abs(budget - gifts_bought.cost);
    }
    ...
    public void make_gift_basket1() {
        ...
        double money_to_spend;
        money_to_spend = partner.maintainence_cost - money_spent;
        for (gift g : gifts) {
            if (money_to_spend > 0 && gifts.indexOf(g) != li && gifts.indexOf(g) != ei && gifts.indexOf(g) != ui) {
                gifts_bought.add(g);
                gifts_bought.cost += g.price;
                gifts_bought.total_value += g.value;
